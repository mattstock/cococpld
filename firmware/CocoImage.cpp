...
uint32_t CocoImage::findSector(uint8_t side, uint8_t track, uint8_t sector) {
  if (sector > sectors_per_track || side >= max_sides)
    return 0;
  return 1L*track*(max_sides*sectors_per_track*sector_size) + 
    1L*side*(sectors_per_track*sector_size) + 
...
boolean VirtualImage::putSector(uint8_t side, uint16_t track, 
				uint16_t sector, char *data) {
  File f;
